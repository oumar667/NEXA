...
const NexaBrain = {
  ...
  async think(message, attachment) {
    ...
    if (
      ...
      if (
        ...
        try {
          ...
          // --------------------------------------------
          // Agent terminé avec une réponse finale
          // --------------------------------------------
          if (
            execution &&
          ...
          // --------------------------------------------
          // Agent terminé en échec :
          // on affiche maintenant la vraie erreur
          // au lieu de masquer le problème.
          // --------------------------------------------
          if (
            execution &&
            execution.state ===
              NexaAgent.STATES.FAILED
          ) {
            console.error(
              "NexaAgent failed :",
              execution.error
          ...
          // --------------------------------------------
          // Aucun résultat final et aucun état FAILED
          // --------------------------------------------
          console.warn(
            "NexaAgent n'a produit aucune réponse finale.",
            execution
          );

          return (
            "L'Agent a terminé son exécution, " +
            "mais aucune réponse finale n'a été produite."
          );
          ...
          return (
            "J'ai rencontré un problème pendant " +
            "l'exécution du plan : " +
            (
              error && error.message
                ? error.message
    ...
    if (attachment) {
      const ack =
        ...
        attachment.name +
        " ». " +
        "Je ne sais pas encore l'analyser en détail, " +
        "mais c'est noté.";
    ...
    // 2) Si aucune règle ne correspond,
    // le modèle IA choisit l'outil
    if (reply === null) {
  ...
  formatPlace(str) {
    ...
    if (
      /,|\s(?:en|au|aux|dans)\s/i.test(text)
    ) {
      return text;
    }
  ...
  async routeWithAI(text) {
    ...
    if (notes.length > 0) {
      ...
      system +=
        " Utilise ces informations quand c'est pertinent.";
    }
    ...
    const raw =
      await NexaAI.ask(messages);

    const decision =
      this.parseRouterJSON(raw);
    ...
    if (decision.tool === "meteo") {
      ...
      return await this.weatherReply(
        city
      );
    }

    if (
      decision.tool === "wikipedia"
    ) {
      const topic =
        (decision.argument || "").trim();
      ...
      return await this.wikiReply(
        topic
      );
    }

    if (
      decision.tool === "calcul"
    ) {
      const expr =
        (decision.argument || "").trim();
  ...
  async decide(text) {
    ...
    if (mentionsKey) {
      if (
        typeof NexaAI === "undefined"
      ) {
        return "Mon module IA n'est pas connecté.";
      }

      if (
        /supprim|efface|oublie|retire|enl[eè]ve/i.test(
          lower
        )
      ) {
        NexaAI.clearKey();
      ...
      if (
        /chang|modifi|nouvelle|remplac|ajoute|saisi|entre|mets/i.test(
          lower
        )
      ) {
        const oldKey =
    ...
    if (
      /(efface|supprime|vide|nettoie|oublie|r[ée]initialise)\s+(?:toute\s+|tout\s+)?(?:la\s+|notre\s+|cette\s+|l')\s*(?:conversation|historique)/i.test(
        lower
      )
    ) {
      if (!hasMemory) {
    ...
    // --- Tool : chronomètre ---
    if (
      /chronom[eè]tre|\bchrono\b|\btimer\b/i.test(
        lower
      )
    ) {
      const r =
    ...
    if (
      ...
      return (
        "C'est fait. J'ai oublié vos notes (" +
        notes.length +
        ")."
      );
    }
    ...
    // --- Oublier la ville ---
    if (
      lower.includes("oublie ma ville")
    ) {
      if (!hasMemory) {
        return "Ma mémoire n'est pas encore connectée.";
      }
    ...
    const asksWeather =
      /m[ée]t[ée]o(?![a-zà-ÿ])|quel temps|quelle temp[ée]rature fait/i.test(
        clean
      );

    if (asksWeather) {
      if (
        /demain|semaine|week-end|weekend/i.test(
          lower
        )
      ) {
